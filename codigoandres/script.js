...
function ordenarFecha() {
  arr.sort((a, b) => parseFloat(a.date) - parseFloat(b.date));
}

function createCard(game) {
  const { date, title, image, text } = game;

  const ficha = document.createElement("article");

  const fechaH2 = document.createElement("h2");

  fechaH2.textContent = date;
  ...
  ficha.append(fechaH2, tituloH3, imagen, textoP);
...
function addUsers(num) {
  const gameArr = getUsers(num);

  const fragment = document.createDocumentFragment();

  for (const ficha of gameArr) {
    const gameCard = createCard(ficha);

    fragment.append(gameCard);
  }
